...
pred_length = 4 # Length of sequence of action to predict
action_length = 2
...
def create_dataset(args):
    ...
    for i in range(len(gripper_poses)):
        ...
        # for j in range(obs_length - 1, demo_length - 2):
            ...
            # pos_noise = 0.1
            # angle_noise = 0.2
            # gen_noise = np.random.rand(obs.shape[0])
            # noise_scale = np.array([pos_noise, pos_noise, angle_noise] * obs_length)
            # gen_noise = (gen_noise - 0.5) * 2.0 * noise_scale
            # obs = obs + gen_noise
...
if __name__ == "__main__":
    ...
    if args.visualization:
        ...
        while True:
            ...
            # NOTE: method 1: choose a random action
